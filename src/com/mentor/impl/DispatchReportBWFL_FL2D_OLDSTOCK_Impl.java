...
import com.mentor.action.DispatchReportBWFL_FL2D_OLDSTOCK_Action;
import com.mentor.resource.ConnectionToDataBase;
import com.mentor.utility.Constants;
import com.mentor.utility.Utility;

public class DispatchReportBWFL_FL2D_OLDSTOCK_Impl {


	// ------------------------------get fl2d list---------------------

	public ArrayList getFL2DList(DispatchReportBWFL_FL2D_OLDSTOCK_Action act) {
	...
	public ArrayList getBWFLList(DispatchReportBWFL_FL2D_OLDSTOCK_Action act, String val)
	{
	...
	public void printReport(DispatchReportBWFL_FL2D_OLDSTOCK_Action act)
	{
		...
		try {
			...
			if (act.getRadio().equalsIgnoreCase("CD"))
			{
				reportQuery = 	" SELECT x.dt, SUM(x.bwfl_dispatchd_bottl) as bwfl_dispatchd_bottl,  " +
								...
								" FROM bwfl_license.fl2_stock_trxn_bwfl_old_stock a, distillery.packaging_details j  " +
								...
								" FROM fl2d.fl2d_stock_trxn_oldstock a, distillery.packaging_details j, licence.fl2_2b_2d m  " +
								" WHERE a.dt BETWEEN '"+ Utility.convertUtilDateToSQLDate(act.getFromDate())+ "' " +
								" AND  '"+ Utility.convertUtilDateToSQLDate(act.getToDate())+ "' " +
			...
			{
				reportQuery = 	" SELECT x.dt, x.bwfl_name, x.vch_license_type, x.licence_nmbr, " +
								...
								" FROM fl2d.fl2d_stock_trxn_oldstock a, distillery.packaging_details j, licence.fl2_2b_2d m, public.district n  " +
								" WHERE a.dt BETWEEN '"+ Utility.convertUtilDateToSQLDate(act.getFromDate())+ "'  " +
								" AND  '"+ Utility.convertUtilDateToSQLDate(act.getToDate())+ "' " +
								" AND  a.int_pckg_id=j.package_id AND a.int_fl2d_id=m.int_app_id AND m.vch_approval='V' "+
			...
			if (rs.next()) {
				...
				if (act.getRadio().equalsIgnoreCase("CD")) {
					jasperReport = (JasperReport) JRLoader.loadObject(relativePath + File.separator+ "DispatchStockReport_BWFL_FL2D_OLDSTOCK.jasper");
				} else if (act.getRadio().equalsIgnoreCase("BFW") || act.getRadio().equalsIgnoreCase("SBW") || act.getRadio().equalsIgnoreCase("SFL")) {
					jasperReport = (JasperReport) JRLoader.loadObject(relativePath + File.separator+ "DispatchStockReportNameWiseOLDSTOCK.jasper");
				} 
				...
				JasperExportManager.exportReportToPdfFile(print,relativePathpdf + File.separator+ "DispatchStockReport_BWFL_FL2D_OLDSTOCK" + "-" + n + ".pdf");
				act.setPdfname("DispatchStockReport_BWFL_FL2D_OLDSTOCK" + "-" + n + ".pdf");
	...
	public boolean write(DispatchReportBWFL_FL2D_OLDSTOCK_Action act)
	{
		...
		try {
			...
			cellhead0.setCellValue(" OldStock Date Wise Dispatches Of BWFL / FL2D From "+ " " + Utility.convertUtilDateToSQLDate(act.getFromDate())+ " " 
			...
			fileOut = new FileOutputStream(relativePath+ "//ExciseUp//MIS//Excel//" + n+ "DispatchStockReport_BWFL_FL2D_OLDSTOCK.xls");
			act.setExlname(n + "DispatchStockReport_BWFL_FL2D_OLDSTOCK");
	...
	public boolean writeNameWise(DispatchReportBWFL_FL2D_OLDSTOCK_Action act)
	{
		...
		try {			
			...
			XSSFRow rowhead0 = worksheet.createRow((int) 0);
			XSSFCell cellhead0 = rowhead0.createCell((int) 0);
			cellhead0.setCellValue(" OldStock Dispatches Of BWFL / FL2D From "+ " " + Utility.convertUtilDateToSQLDate(act.getFromDate())+ " " + 
			" To " + " "+ Utility.convertUtilDateToSQLDate(act.getToDate()));
			...
			Random rand = new Random();
			int n = rand.nextInt(550) + 1;
			fileOut = new FileOutputStream(relativePath+ "//ExciseUp//MIS//Excel//" + n+ "_DispatchStockReportNameWiseOLDSTOCK.xls");
			act.setExlname(n + "_DispatchStockReportNameWiseOLDSTOCK");
